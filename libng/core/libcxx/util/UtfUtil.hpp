...
template<class DST, class SRC> LIBNG_INLINE
...
uint32_t UtfUtil::_decodeUtf(const char* & src, const char* end) {
	...
	if ((v & 0xE0) == 0xC0) {
		if(src + 2 > end) throw LIBNG_ERROR("");
	...
	if ((v & 0xF0) == 0xE0) {
		if(src + 3 > end) throw LIBNG_ERROR("");
	...
	if ((v & 0xF8) == 0xF0) {
		if(src + 4 > end) throw LIBNG_ERROR("");
	...
	if ((v & 0xFC) == 0xF8) {
		if(src + 5 > end) throw LIBNG_ERROR("");
	...
	if ((v & 0xFE) == 0xFC) {
		if(src + 6 > end) throw LIBNG_ERROR("");
...
uint32_t UtfUtil::_decodeUtf(const char16_t*& src, const char16_t* end) {
	...
	if (v >= 0xD800 && v < 0xDBFF) {
		if(src + 2 > end) throw LIBNG_ERROR("Out of Range");
...
template<class OUT_STR> LIBNG_INLINE
