...
void Material_DX11::_bindStageHelper(RenderContext_DX11* ctx, STAGE* stage) {
  ...
  for (auto& cb : stage->constBuffers()) {
    ...
    auto* gpuBuffer = static_cast<GPUBuffer_DX11*>(cb.gpuBuffer.ptr());
    if (!gpuBuffer) {
      throw LIBNG_ERROR("cosnt buffer is null");
    }

    auto* d3dBuf = gpuBuffer->d3dBuf();
    if (!d3dBuf) {
      throw LIBNG_ERROR("d3dbuffer is null");
    }
  ...
  for (auto& texParam : stage->texParams()) {
    ...
    switch (texParam.dataType()) {
      ...
      default: throw LIBNG_ERROR("bind unsupported texture type");
    }
  }
}

void Material_DX11::PixelStage_DX11::bind(RenderContext_DX11* ctx, const VertexLayout* vertexLayout) {
  _bindStageHelper(ctx, this);
}

void Material_DX11::VertexStage_DX11::bind(RenderContext_DX11* ctx, const VertexLayout* vertexLayout) {
  _bindStageHelper(ctx, this);
  bindInputLayout(ctx, vertexLayout);
}

void Material_DX11::VertexStage_DX11::bindInputLayout(RenderContext_DX11* ctx, const VertexLayout* vertexLayout) {
  if (!vertexLayout)
    throw LIBNG_ERROR("vertexLayout is null");
  ...
  if (it != _inputLayoutsMap.end()) {
    ...
    for (auto& input : vsInfo->inputs) {
      ...
      if (!e) {
        throw LIBNG_ERROR("vertex sematic {} not found", input.semantic);
      }
    ...
    auto* ss      = static_cast<Shader_DX11::VertexStage_DX11*>(_shaderStage);
