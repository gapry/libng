...
bool pngLoader::Reader::error_longjmp_restore_point() {
...
void pngLoader::Reader::load(Image& img, ByteSpan data, ColorType expectType) {
  _data    = data;
  _readPtr = data.data();

  if (data.size() < 8 || png_sig_cmp(data.data(), 0, 8)) {
    throw LIBNG_ERROR("not png format");
  }

  LIBNG_ASSERT(!_png);
  _png = png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, nullptr, nullptr);
  if (!_png) {
    throw LIBNG_ERROR("png error read struct");
  }

  _info = png_create_info_struct(_png);
  if (!_info) {
    throw LIBNG_ERROR("png error create info struct");
  }

  png_set_read_fn(_png, this, &s_onRead);

  if (error_longjmp_restore_point()) {
    throw LIBNG_ERROR("png error read header");
  }
  ...
  if (error_longjmp_restore_point()) {
    throw LIBNG_ERROR("png error decompress");
  }
    ...
    if (in_bit == 8) {
      switch (in_color_type) {
        case PNG_COLOR_TYPE_RGB:
          ...
          break; // DirectX doesn't support RGBb, so convert to RGBAb instead
        ...
        case PNG_COLOR_TYPE_PALETTE: {
          ...
          if (error_longjmp_restore_point()) {
            throw LIBNG_ERROR("png error read header color type");
          }
      ...
      switch (in_color_type) {
        case PNG_COLOR_TYPE_RGB:
          out_color_type = ColorType::RGBAs;
          break; // DirectX doesn't support RGBb, so convert to RGBAb instead
      ...
      throw LIBNG_ERROR("png error unsupported format");
    }
  }

  if (out_color_type == ColorType::None) {
    throw LIBNG_ERROR("png error unsupported output format");
  }
  ...
  if (width <= 0 || height <= 0) {
    throw LIBNG_ERROR("png error width <= 0 or height <= 0");
  }
  ...
  if (error_longjmp_restore_point()) {
    throw LIBNG_ERROR("png error read pixels");
  }
  png_read_image(_png, rows.data());
}

void pngLoader::Reader::s_onRead(png_structp png, png_bytep dest, png_size_t len) {
  auto* p = reinterpret_cast<Reader*>(png_get_io_ptr(png));
  if (!p) {
    throw LIBNG_ERROR("png error read data");
  }
  p->onRead(dest, len);
}

void pngLoader::Reader::onRead(png_bytep dest, png_size_t len) {
  size_t n = len;
  if (_readPtr + n > _data.end()) {
    throw LIBNG_ERROR("png error read data out of range");
  }
  memcpy(dest, _readPtr, n);
  _readPtr += n;
}

void pngLoader::Reader::setReadFilter(ColorType out_color_type,
                                      int in_color_type,
                                      int in_bit,
                                      bool in_palette_has_alpha) {
  ...
  switch (elementType) {
    case ColorElementType::UNorm8: {
      switch (in_bit) {
        ...
        default: throw LIBNG_ERROR("png error unsupported format");
      }
    } break;
  ...
  switch (out_color_model) {
    ...
    case ColorModel::RGBA: {
      if (!(in_color_type & PNG_COLOR_MASK_COLOR)) {
