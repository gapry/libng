...
void WavefrontObjLoader::_parseLine() {
  ...
  if (_token == "v")
    return _parseLine_v();
  if (_token == "vt")
    return _parseLine_vt();
  if (_token == "vn")
    return _parseLine_vn();
  if (_token == "f")
    return _parseLine_f();
}
...
void WavefrontObjLoader::_parseLine_v() {
  ...
  Tuple3f pos(value.x / value.w, value.y / value.w, value.z / value.w);
...
void WavefrontObjLoader::_parseLine_f() {
  ...
  while (_lineRemain.size() > 0) {
    ...
    if (vi != 0) {
      if (vi > 0) {
        ...
        vi = static_cast<int>(_tmpPos.size()) + vi;
      }
    ...
    if (vt != 0) {
      if (vt > 0) {
        ...
        vt = static_cast<int>(_tmpUv.size()) + vt;
      }
    ...
    if (vn != 0) {
      if (vn > 0) {
        ...
        vn = static_cast<int>(_tmpNormal.size()) + vn;
      }
  ...
  for (size_t i = 0; i < face_vi.size(); i++) {
    ...
    if (vi >= _outMesh->pos.size()) {
      throw LIBNG_ERROR("vi out of range");
    }
