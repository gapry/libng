...
float4x4  libng_matrix_model;
float4x4  libng_matrix_view;
float4x4  libng_matrix_proj;
float4x4  libng_matrix_mvp;

float3    libng_camera_pos;
float3    libng_light_pos;
float3    libng_light_dir;
float     libng_light_power;
float3    libng_light_color;
...
PixelIn vs_main(VertexIn i) {
  PixelIn o;
  o.positionWS  = mul(libng_matrix_model, i.positionOS);
  o.positionHCS = mul(libng_matrix_mvp,   i.positionOS);
...
float3 lighting_blinn_phong(Surface s) {
  ...
  float3 lightDir      = s.positionWS - libng_light_pos;
  float  lightDistance = length(libng_light_dir);
  lightDir = normalize(lightDir);

  float lambertian = max(dot(normal, -libng_light_dir), 0);
  float specularPower = 0;

  if (lambertian > 0) {
    float3 viewDir = normalize(libng_camera_pos - s.positionWS);
  ...
  float3 outLightColor = libng_light_color * libng_light_power / (lightDistance * lightDistance);
