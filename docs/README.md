## [Khronos](https://www.khronos.org/)
- [Khronos Registry](https://www.khronos.org/registry/)
- [glTF](https://www.khronos.org/gltf/)

## [Vulkan](https://www.vulkan.org/)
- [One stop solution for all Vulkan samples](https://github.com/khronosGroup/Vulkan-samples)
- [SPIR Overview](https://www.khronos.org/spir/)
- [SPIR-V Tools](https://github.com/KhronosGroup/SPIRV-Tools)

## [OpenGL](https://www.opengl.org/)
- [Getting Started](https://www.khronos.org/opengl/wiki/Getting_Started)
- [Category:Example Code](https://www.khronos.org/opengl/wiki/Category:Example_Code)
- [Common Mistakes](https://www.khronos.org/opengl/wiki/Common_Mistakes)
- [MSDN OpenGL](https://docs.microsoft.com/en-us/windows/win32/opengl/opengl?redirectedfrom=MSDN)
- [Shader Compilation](https://www.khronos.org/opengl/wiki/Shader_Compilation)

## [Direct3D](https://docs.microsoft.com/en-us/windows/win32/directx)
- [DirectX 11](https://docs.microsoft.com/en-us/windows/win32/direct3d11/atoc-dx-graphics-direct3d-11)
  - [Programming guide for Direct3D 11](https://docs.microsoft.com/en-us/windows/win32/direct3d11/dx-graphics-overviews)
  - [Direct3D game sample](https://docs.microsoft.com/en-us/samples/microsoft/windows-universal-samples/simple3dgamedx/)
  - [DirectX and XAML game sample](https://docs.microsoft.com/en-us/samples/microsoft/windows-universal-samples/simple3dgamexaml/)
- [DirectX 12](https://docs.microsoft.com/en-us/windows/win32/direct3d12/direct3d-12-graphics)
  - [DirectX Graphics Samples](https://github.com/microsoft/DirectX-Graphics-Samples)
  - [DirectML](https://github.com/microsoft/DirectML)
- Shader Compiler
  - [D3DCompiler Reference](https://docs.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-d3dcompiler-reference)
  - [D3DReflect function (`d3dcompiler.h`)](https://docs.microsoft.com/en-us/windows/win32/api/d3dcompiler/nf-d3dcompiler-d3dreflect)
  - [DirectX Shader Compiler](https://github.com/microsoft/DirectXShaderCompiler)
- HLSL
  - [rcp (sm5 - asm)](https://docs.microsoft.com/en-us/windows/win32/direct3dhlsl/rcp--sm5---asm-)
- [DirectSound](https://docs.microsoft.com/en-us/previous-versions/windows/desktop/bb318665(v=vs.85))

## [Metal](https://developer.apple.com/documentation/metal/)
- [Metal Sample Code Library](https://developer.apple.com/documentation/metal/metal_sample_code_library?language=objc)

## Unity
- [Multiplayer and Networking](https://docs.unity3d.com/Manual/UNet.html)
- [CommandBuffer](https://docs.unity3d.com/ScriptReference/Rendering.CommandBuffer.html)
- [URP ShaderLab Pass tags](https://docs.unity3d.com/Packages/com.unity.render-pipelines.universal@11.0/manual/urp-shaders/urp-shaderlab-pass-tags.html)
- [NormalMapEncoding.DXT5nm](https://docs.unity3d.com/2021.1/Documentation/ScriptReference/NormalMapEncoding.DXT5nm.html)
- [Camera.RenderToCubemap](https://docs.unity3d.com/ScriptReference/Camera.RenderToCubemap.html)

## Unreal Engine
- [Networking and Multiplayer](https://docs.unrealengine.com/5.0/en-US/networking-and-multiplayer-in-unreal-engine/)
- [Materials](https://docs.unrealengine.com/5.0/en-US/unreal-engine-materials/)
- [Adding Global Shaders to Unreal Engine](https://docs.unrealengine.com/5.0/en-US/adding-global-shaders-to-unreal-engine/)
- [FName](https://docs.unrealengine.com/4.27/en-US/ProgrammingAndScripting/ProgrammingWithCPP/UnrealArchitecture/StringHandling/FName/)

## NVIDIA
- [CUDA C++ Programming Guide](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html)
- [Nsight Compute CLI](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html)

## Linux
- [poll(2) — Linux manual page](https://man7.org/linux/man-pages/man2/poll.2.html)
- [epoll(7) — Linux manual page](https://man7.org/linux/man-pages/man7/epoll.7.html)
- [The rapid growth of `io_uring`](https://lwn.net/Articles/810414/)

## FreeBSD
- [KQUEUE(2) FreeBSD System Calls Manual](https://www.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2)
- [POLL(2) FreeBSD System Calls Manual](https://www.freebsd.org/cgi/man.cgi?poll)

## macOS
- [POLL(2) BSD System Calls Manual](https://developer.apple.com/library/archive/documentation/System/Conceptual/ManPages_iPhoneOS/man2/poll.2.html)

## Windows
- [`WSAPoll` function (`winsock2.h`)](https://docs.microsoft.com/en-us/windows/win32/api/winsock2/nf-winsock2-wsapoll)
- [I/O Completion Ports](https://docs.microsoft.com/en-us/windows/win32/fileio/i-o-completion-ports)

## Android
- [Android Developers > NDK > Reference > Networking](https://developer.android.com/ndk/reference/group/networking)

## Web
- [The WebSocket API (WebSockets)](https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API)
- [WebGL](https://www.khronos.org/webgl/)
  - [WebGL: 2D and 3D graphics for the web](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API)
- [WebGPU](https://www.w3.org/TR/webgpu/)
  - [A Taste of WebGPU in Firefox](https://hacks.mozilla.org/2020/04/experimental-webgpu-in-firefox/)
  - [Access modern GPU features with WebGPU](https://developer.chrome.com/docs/web-platform/webgpu/)
- [WebAssembly](https://webassembly.org/)

## Third Party
- [zstd](https://github.com/facebook/zstd)
- [assimp](https://github.com/assimp/assimp)
- [FreeType](https://github.com/freetype)
- [libpng](http://www.libpng.org/pub/png/libpng.html)
- [libjpeg](http://libjpeg.sourceforge.net/)
- [OpenAL](https://www.openal.org/documentation/)
- [FFmpeg](https://ffmpeg.org/documentation.html)
- [Bullet Physics](https://github.com/bulletphysics/bullet3/tree/master/examples) 
- [POCO C++ Libraries](https://github.com/pocoproject/poco)
- [ImGui](https://github.com/ocornut/imgui)
- [Apache Portable Runtime (APR) project](https://apr.apache.org/) / [GitHub](https://github.com/apache/apr)
- [Facebook Folly](https://github.com/facebook/folly)
- [Google Abseil](https://abseil.io/) / [GitHub](https://github.com/abseil/abseil-cpp)
- [wxWidget wxBase/wxCore](https://docs.wxwidgets.org/3.0/page_libs.html) / [GitHub](https://github.com/wxWidgets)
- [Qt core](https://doc.qt.io/qt-6/qtcore-index.html) / [corelib (GitHub)](https://github.com/qt/qtbase/tree/dev/src/corelib)
- [EABase](https://github.com/electronicarts/EABase)
- [EASTL](https://github.com/electronicarts/EASTL)
- [EAThread](https://github.com/electronicarts/EAThread)
- [EAStdC](https://github.com/electronicarts/EAStdC)
- [LZ4](https://github.com/lz4/lz4)

## Shader
- [Unity Rendering Tutorials](https://catlikecoding.com/unity/tutorials/rendering/)
- [Shader Tutorials by Ronja](https://www.ronja-tutorials.com/)
- [Normal Map Compression](http://wiki.polycount.com/wiki/Normal_Map_Compression)
- [Scriptable Renderer Feature](https://samdriver.xyz/article/scriptable-render)
- [5 ways to draw an outline](https://alexanderameye.github.io/notes/rendering-outlines/)

## Tools
- [Make for Windows](http://gnuwin32.sourceforge.net/packages/make.htm)
- [Wireshark](https://wiki.wireshark.org/Home)
- [RenderDoc](https://renderdoc.org/docs/index.html)

## Engine
- [RakNet](https://github.com/facebookarchive/RakNet)
- [O3DE](https://github.com/o3de/)
- [Godot](https://github.com/godotengine)

## [GDC](https://gdconf.com/)
- [Parallelizing the Naughty Dog Engine Using Fibers](https://www.gdcvault.com/play/1022186/Parallelizing-the-Naughty-Dog-Engine)
- [Destiny's Multithreaded Rendering Architecture](https://www.youtube.com/watch?v=0nTDFLMLX9k)
- [Low Complexity, High Fidelity: The Rendering of INSIDE](https://www.youtube.com/watch?v=RdN06E6Xn9E)

## [ACM SIGGRAPH](https://www.siggraph.org/)
- [SIGGRAPH 2022](http://kesen.realtimerendering.com/sig2022.html)
- [SIGGRAPH 2005](http://kesen.realtimerendering.com/sig2005.html)
- [Advances in Real-Time Rendering in 3D Graphics and Games](https://advances.realtimerendering.com/)
- [Physics and Math of Shading](https://www.youtube.com/watch?v=j-A0mwsJRmk)
- [Moving Mobile Graphics](https://www.youtube.com/watch?v=wOBCWZJq6zs)

## References
- [A Study Path for Game Programmer](https://github.com/miloyip/game-programmer)
- [Graphics Programming Weekly](https://www.jendrikillner.com/tags/weekly/)
- [Graphics Studies Compilation](https://www.adriancourreges.com/blog/)
- [bitsquid: development blog](https://bitsquid.blogspot.com/)
- [Towards sockets and networking in WebAssembly and WASI](https://radu-matei.com/blog/towards-sockets-networking-wasi/)
- [Porting lwIP](https://docs.aws.amazon.com/freertos/latest/portingguide/porting-lwip.html)
- [Boost.Process Design overview](http://www.highscore.de/boost/process/process/design.html#process.design.steps)
